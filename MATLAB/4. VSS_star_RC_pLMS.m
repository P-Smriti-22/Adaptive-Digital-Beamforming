...
for j=1:N_Sim
    ...
    d = awgn(sd,10,'measured'); %Desired signal 7 1 -5 dB
    ...
    w = zeros(N,1); 
    e = zeros(N_Samples,1); del_e = zeros(N_Samples,1);
    ...
    mu =2^floor(-log2(del_e(1)^2+sigma_square(1)));
    MU(j,1) = mu;
    w = w + mu*conj(e(1)).*x(:,1); 
    for i=2:N_Samples
        s(i) = d(i) - 1i*d(i-1);
        ...
        mu =2^floor(-log2(del_e(i)^2+sigma_square(i)));
        MU(j,i) = mu;
        w = w + mu*conj(e(i)).*x(:,i);
    end
    E(j,:) = e;
...
title('Beam Pattern'); ylabel('Magnitude (dB)'); xlabel('Angle of Arrival (Degrees)');
